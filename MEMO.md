...
- **AI SDK: Vercel AI SDK**（`ai` パッケージ）
  - Google Gemini / Anthropic Claude を同一インターフェースで切り替え可能
  - デフォルトは **Google Gemini**（無料枠があるため）
  - `--provider anthropic` でClaude切替可能
  - 環境変数: `GOOGLE_GENERATIVE_AI_API_KEY` / `ANTHROPIC_API_KEY`
...
- [x] プレイヤーデータ（`/player`）の整形：どのフィールドを抽出するか決める（Step 3で実装済み）
- [x] RotE TBの手動JSON設計：小隊情報をどう構造化するか（Step 4でスキーマ設計済み、実データ未投入）
- [x] Claude APIへのプロンプト設計：どう渡すと良いアドバイスが返るか（Step 5で初版実装済み）
- [ ] `rote-platoons.json` / `rote-special-missions.json` への実データ投入
...
#### Step 4: `core/data` — RotE TB 手動JSON設計・作成 (`feat/step4-rote-data`) ✅（スキーマのみ、実データ未投入）

- `packages/core/data/rote-platoons.json` : 小隊情報（キャラID + 必要レリック数）
- `packages/core/data/rote-special-missions.json` : スペシャルミッション情報
- ゲームアップデート時に手動更新する運用

#### Step 5: `core/advisor` — AI プロンプト生成・呼び出し (`feat/step5-advisor`) ✅

- `packages/core/advisor/prompt.ts` : RotE TB用プロンプトを組み立てる関数
- `packages/core/advisor/client.ts` : Vercel AI SDK を用いて AI プロバイダ（Anthropic Claude / Google Gemini）を呼び出すクライアント
- `.env` に `ANTHROPIC_API_KEY` および `GOOGLE_GENERATIVE_AI_API_KEY` を設定する運用（利用するプロバイダに応じて参照）
...
- **Step 5**: Vercel AI SDK 経由で Claude / Gemini を試しつつプロンプトを反復して改善する（プロバイダ切り替え戦略も検討）
