...
## Phase 1 CLI チャット形式への改修設計（決定済み）

### 背景・問題意識

Phase 1の初期実装（`bun run cli 445833733 --tb rote`）は動作したが以下の問題があった：
...
- [ ] `packages/cli/config.ts` を実装（アライコードの保存・読み込み）
- [ ] `packages/cli/selector.ts` を実装（選択式UIのユーティリティ）
- [ ] `packages/cli/modes/rote.ts` を実装（RotE TBの目的選択肢定義）
- [ ] `packages/core/comlink/formatPlayer.ts` にGP上位N件抽出関数を追加
- [ ] `packages/core/advisor/prompt.ts` をチャット用システムプロンプトに変更
- [ ] `packages/core/advisor/client.ts` を会話履歴対応に変更
- [ ] `packages/cli/chat.ts` を実装（対話ループ本体）
- [ ] `packages/cli/index.ts` を全面改修（チャットループ起動）
- [ ] 手動JSONへの実データ入力（後回し可・埋まれば精度UP）
